        ...
        while True:
            try:
                ...
                if message:
                    ...
                    if (message['channel'].decode("utf-8")=="tickets_done"):
                ...
                while True:
                    ...
                    try:
                        ...
                        self.pubsub.subscribe(['last_session'],['tickets_done'])
                        break
            time.sleep(0.001)  # be nice to the system :)

client = Listener1(r, ['last_session','tickets_done'])
client.start()

async def showList(q: Q):
    global data_rows, columnsDone
...
async def tickets_done(q: Q):
    print(str("starting tickets_done app..."))
...
#########   U   P   D   A   T   E     T   A   B   L   E   ############
    if q.args.btnUpd:
        data_rows = getAllTickets(r, 'tickets_key', 'tickets_', 'done')
...
@app('/tickets_done', mode = 'unicast')
async def team1(q: Q):
    route = q.args['#']
    await tickets_done(q)