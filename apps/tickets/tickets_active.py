        ...
        while True:
            try:
                ...
                if message:
                    ...
                    if (message['channel'].decode("utf-8")=="tickets"):
                        self.work2(message['data'])
                    if (message['channel'].decode("utf-8")=="tickets_return"):
                ...
                while True:
                    ...
                    try:
                        ...
                        self.pubsub.subscribe(['last_session'],['tickets'],['tickets_return'])
                        break
            time.sleep(0.001)  # be nice to the system :)

client = Listener1(r, ['last_session','tickets','tickets_return'])
client.start()
...
async def tickets_active(q: Q):
    print(str("starting tickets_active app..."))
...
#########   U   P   D   A   T   E     T   A   B   L   E   ############
    if q.args.btnUpd:
        data_rows = getAllTickets(r, 'tickets_key', 'tickets_', 'activos')
        ...
        await q.run(showList, q)
        await q.page.save()
######### S   E   N   D   TO    F  I  B  E  R   ############
    if q.args.btnDptoFibra:
        ...
        for i in range(0,len(data_rows_send)):
            ...
            try:
                r.publish("tickets_fiber",json_datos)
                time.sleep(0.3)
            except Exception as e:
                pass
                print(e)
            await q.run(showList,q)
        await q.page.save()
######### S   E   N   D   TO    W   R   L   S   S   ############
    if q.args.btnDptoWrlss:
        eliminados = q.args.issues
        data_rows_temp, data_rows_send, found = [], [], 0
        for y in data_rows:
            for x in eliminados:
        ...
        for i in range(0,len(data_rows_send)):
            ...
            try:
                r.publish("tickets_wireless",json_datos)
                time.sleep(0.3)
            except Exception as e:
                print(e)
            await q.run(showList,q)
        await q.page.save()
######### S   E   N   D   TO    Y  I  K  E  Y   ############
    if q.args.btnDptoYIKey:
        eliminados = q.args.issues
        data_rows_temp, data_rows_send, found = [], [], 0
        for y in data_rows:
            for x in eliminados:
        ...
        for i in range(0,len(data_rows_send)):
            ...
            try:
                r.publish("tickets_yikey",json_datos)
                time.sleep(0.3)
            except Exception as e:
                print(e)
            await q.run(showList,q)
...
######### S   E   N   D   TO    E  X  C     ############
    ...
    if not q.client.initialized:
        ...
        q.page['boton2'] = ui.section_card(
            ...
            subtitle='',
            items=[
                ui.button(name='btnDptoFibra', label='Fibra Óptica', disabled = False, primary=True),
                ui.button(name='btnDptoWrlss', label='Inalámbrico', disabled = False, primary=True),
                ui.button(name='btnDptoYIKey', label='Acceso Público', disabled = False, primary=True),
                ui.button(name='btnDptoExcptns', label='Excepción', disabled = False, primary=True),
            ],
        )
...
@app('/tickets_active', mode = 'unicast')
async def team1(q: Q):
    route = q.args['#']
    await tickets_active(q)