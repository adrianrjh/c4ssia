...
async def paso1(q: Q):
    ...
    q.page['textdeinicio'] = ui.section_card(box=ui.box('der1_10', order=1),title='',subtitle='',items=[ui.text_xl('Registro de Equipos de Red')],)
...
async def paso3(q: Q):
    ...
    q.page['upldCSV'] = ui.section_card(
            ...
            ui.file_upload(name='file_upload',label='Upload!',multiple=False,compact=True,file_extensions=['csv', 'xlsx'],max_file_size=10,max_size=15),
...
async def addDevs(q: Q):
    print(str("starting addDevs..."))
                ...
                for y in data_rows:
                    for x in seleccionated:
                        if str(y[0])==str(x):
                            ...
                            equiposRedMarca = str(y[1])
                            equiposRedModelo = str(y[2])
                            equiposRedDescripcion = str(y[3])
                            equiposRedUnidad = str(y[4])
                            try:
                                key_count=r.get('key_equipos_red')
                                if key_count == None:
                                    key_count = 0
                                    r.set("key_equipos_red",str(key_count))
                                else:
                                    key_count=key_count.decode("utf-8")
                            except Exception as e:
                                print(e)
                            res = getAllEquiposRedxMarca(r, equiposRedMarca, equiposRedModelo)
                            if res == 'NO':
                                if key_count == None:
                                    key_count=1
                                else:
                                ...
                                r.set("key_equipos_red",str(key_count))
                                pp={"marca":str(equiposRedMarca),"modelo":str(equiposRedModelo),"descripcion":str(equiposRedDescripcion),"unidad":str(equiposRedUnidad)}
                                r.hmset('equipos_red'+str(key_count), pp)
                    if found==0:
            ...
            for path in q.args.file_upload:
                ...
                if '.xlsx' in local_path:
                    ...
                    for index, row in df.iterrows():
                        ...
                        if count > 1:
                            data_rows_keycountMat += 1
                            data_rows.append([str(data_rows_keycountMat), row[0], row[1], row[2], row[3]])
        paso1P = 1
        paso2P = 0
        paso3P = 0
        bandPaso1 = 0
