...
async def paso1(q: Q):
    global comboboxUsers, data_rows, data_rows_keycount

    data_rows_keycount = 0

    del q.page['btnAtras1']
    del q.page['table'] 
    del q.page['textQrCodeDevol']
    del q.page['checkRight']
    del q.page['btnPDF']
    del q.page['btnAtrasP1']
    del q.page['textQrCodeDevol']
    del q.page['btnFinDevol']
    ...
    q.page['comboboxproyectos'] = ui.section_card(
        ...
        subtitle='',
        items=[
            ui.combobox(name='comboboxusers', label='Trabajador', value='Seleccionar', choices=comboboxUsers,trigger=True),
            ui.button(name='btnDevolver', label='Devolver', disabled = False, primary=True)
        ],
    )
    await q.page.save()
...
async def paso2(q: Q):
    global data_rows, comboboxMotivos
    ...
    q.page['btnAtrasP1'] = ui.section_card(box=ui.box('izq1_11', order=1),title='',subtitle='',items=[ui.button(name='btnAtrasP1', label='Atrás', disabled = False, primary=True)])
    q.page['textQrCodeDevol'] = ui.section_card(
        box=ui.box('mid1_11', order=1),
        title='',
        subtitle='',
        items=[
            ui.textbox(name='textqrcode', label='QR CODE',trigger=True),
            ui.combobox(name='comboboxmotivo', label='Motivo', value='Seleccionar', choices=comboboxMotivos,trigger=True),
            ui.button(name='btnAgregar', label='Agregar', disabled = False, primary=True)
        ],
    )

    q.page['btnEliminar'] = ui.section_card(box=ui.box('mid1_13', order=1),title='',subtitle='',items=[ui.button(name='btnDelete', label='Eliminar Devolución', disabled = False, primary=True)])
    q.page['btnFinDevol'] = ui.section_card(box=ui.box('mid1_13', order=2),title='',subtitle='',items=[ui.button(name='btnFinDevol', label='Finalizar Devolución', disabled = False, primary=True)])
...
async def paso3(q: Q):
    ...
    q.page['checkRight'] = ui.section_card(box=ui.box(zone='mid1_12', order=1),title='',subtitle='',items=[ui.persona(name="btnDevolTermi",title='Devolución Terminada', subtitle='', caption='', size='xl', image=checkRight_logo)])
...
async def sims_devolucion(q: Q):
    print(str("starting sims_devolucion..."))
    global data_rows, data_rows_keycount, r,  session
    global paso1P, paso2P, paso3P, bandPaso1, bandPaso2, bandPaso3
    global trabajador, qr_code, noserie, proyecto, marcaDev, modeloDev, descripcion, garantia, motivo, ubicacion, status
    ...
    if q.args.comboboxmotivo:
        if q.args.comboboxmotivo != 'Seleccionar':
            motivo = str(q.args.comboboxmotivo)
        await q.page.save()
        ...
        if qr_code != '':
            ...
            if len(data_rows)>0:
                for x in range(0,len(data_rows)):
                    ...
                    data_rows2.append([str(data_rows[x][0]), data_rows[x][1], data_rows[x][2], data_rows[x][3], data_rows[x][4], data_rows[x][5], data_rows[x][6], data_rows[x][7], data_rows[x][8], data_rows[x][9]])
            ...
            try:
                if res != None:
                    ...
                    status = res['status']
                    data_rows.append([str(data_rows_keycount), noserie, proyecto, marcaDev, modeloDev, descripcion, garantia, motivo, ubicacion, status])
                else:
                    qr_code = ''
            except Exception as e:
                print(e)
    ...
    if q.args.btnFinDevol:
        ...
        if selectioned != None:
            if len(selectioned) == len(data_rows):
                ...
                json_datos={
                    'usuarioD':str(data_user[0]),
                    'puestoD':str(data_user[1]),
                    'lista':data_rows
                }
                try:
                    r.publish("yi_pdfs_devolucion",json.dumps(json_datos))
                    time.sleep(0.3)
                except Exception as e:
                    print(e)
                #changeStateArticle(data_rows, trabajador, 'devolver')
                #paso1P = 0
                #paso2P = 0
                #paso3P = 1
                #bandPaso3 = 0
                ...
                zones=[
                    ui.zone('left1_11',size='100%',direction=ui.ZoneDirection.COLUMN,zones=[
                        ...
                        ui.zone('body',size='93%',direction=ui.ZoneDirection.ROW, zones=[
                            ...
                            ui.zone('mid',size='80%',direction=ui.ZoneDirection.COLUMN, zones=[
                                ...
                                ui.zone('mid1_13', align='center', size='20%', direction=ui.ZoneDirection.ROW),
                            ]),
...
@app('/sims_devolucion', mode = 'unicast')
async def team1(q: Q):
    route = q.args['#']
    await sims_devolucion(q)
