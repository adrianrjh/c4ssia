...
async def paso1(q: Q):
    ...
    del q.page['table']
    del q.page['textQrCodeAsigna']
    del q.page['btnFinAsig']
    ...
    q.page['comboboxproyectos'] = ui.section_card(
        ...
        items=[
            ...
            ui.button(name='btnAsignar', label='Asignar', disabled = False, primary=True)
        ],
    )
    await q.page.save()
...
async def paso2(q: Q):
    ...
    q.page['textQrCodeAsigna'] = ui.section_card(
        box=ui.box('mid1_11', order=1),
        title='',
        subtitle='',
        items=[
    ...
    q.page['btnEliminar'] = ui.section_card(box=ui.box('mid1_13', order=1),title='',subtitle='',items=[ui.button(name='btnDelete', label='Eliminar Asignación', disabled = False, primary=True)])
    q.page['btnFinAsig'] = ui.section_card(box=ui.box('mid1_13', order=2),title='',subtitle='',items=[ui.button(name='btnFinAsig', label='Finalizar Asignación', disabled = False, primary=True)])
...
async def paso3(q: Q):
    ...
    q.page['checkRight'] = ui.section_card(box=ui.box(zone='mid1_12', order=1),title='',subtitle='',items=[ui.persona(name="btnAsigTermi",title='Asignación Terminada', subtitle='', caption='', size='xl', image=checkRight_logo)])
...
async def sims_asignacion(q: Q):
    print(str("starting sims_asignacion..."))
    ...
    if q.args.btnFinAsig:
        ...
        if selectioned != None:
            if len(selectioned) == len(data_rows):
                ...
                try:
                    r.publish("yi_pdfs_asignacion",json.dumps(json_datos))
                    time.sleep(0.3)
                except Exception as e:
                    print(e)
                #changeStateArticle(data_rows, trabajador, 'asignar')
                #paso1P = 0
                #paso2P = 0
                #paso3P = 1
                #bandPaso3 = 0
                ...
                ui.zone('left1_11',size='100%',direction=ui.ZoneDirection.COLUMN,zones=[
                    ...
                    ui.zone('body',size='93%',direction=ui.ZoneDirection.ROW, zones=[
                        ...
                        ui.zone('mid',size='76%',direction=ui.ZoneDirection.COLUMN, zones=[
                            ...
                            ui.zone('mid1_12', size='60%', direction=ui.ZoneDirection.ROW),
...
@app('/sims_asignacion', mode = 'unicast')
async def team1(q: Q):
    route = q.args['#']
    await sims_asignacion(q)
