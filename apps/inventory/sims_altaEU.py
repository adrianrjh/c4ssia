...
async def paso1(q: Q):
    ...
    global marcaEU, modeloEU, descripcionEU, garantiaEU, noserieEU, motivoEU
    ...
    q.page['combotextboxes'] = ui.section_card(
        ...
        subtitle='',
        items=[
            ui.combobox(name='combomarca', label='Marca', value=str(marcaEU), choices=comboboxMarca,trigger=True, disabled=False),
            ui.combobox(name='combomodelo', label='Modelo', value=str(modeloEU), choices=comboboxModelo,trigger=True, disabled=False),
            ui.combobox(name='combodescripcion', label='Descripción', value=str(descripcionEU), choices=comboboxDescripcion,trigger=True, disabled=False),
            ui.combobox(name='combogarantia', label='Garantia', value=str(garantiaEU), choices=comboboxGaran,trigger=True, disabled=False),
            ui.combobox(name='combomotivos', label='Motivo', value=str(motivoEU), choices=comboboxMotivos,trigger=True, disabled=False),
...
async def sims_altaEU(q: Q):
    print(str("starting sims_altaEU..."))
    ...
    if 'combomarca' in q.args:
        if q.args.combomarca and str(q.args.combomarca) != 'Seleccionar':
            marcaEU = str(q.args.combomarca)
            res = getAllDevsMarca(r, marcaEU)
            modeloBefore = ''
            comboboxModelo = []
            for x in range(0, len(res)):
    ...
    if 'combomodelo' in q.args:
        if q.args.combomodelo and str(q.args.combomodelo) != 'Seleccionar':
            modeloEU = str(q.args.combomodelo)
            res = getAllDevsModelo(r, modeloEU)
            descripcionBefore = ''
            comboboxDescripcion = []
            for x in range(0, len(res)):
    ...
    if 'combodescripcion' in q.args:
        if q.args.combodescripcion and str(q.args.combodescripcion) != 'Seleccionar':
            descripcionEU = str(q.args.combodescripcion)
            paso1P = 1
            paso2P = 0
            paso3P = 0
            paso4P = 0
    ...
    if 'combogarantia' in q.args:
        if q.args.combogarantia and str(q.args.combogarantia) != 'Seleccionar':
            garantiaEU = str(q.args.combogarantia)
            paso1P = 1
            paso2P = 0
            paso3P = 0
            paso4P = 0
    ...
    if 'combomotivos' in q.args:
        if q.args.combomotivos and str(q.args.combomotivos) != 'Seleccionar':
            motivoEU = str(q.args.combomotivos)
            paso1P = 1
            paso2P = 0
            paso3P = 0
            paso4P = 0
    ...
    if 'textnoserie' in q.args:
        if q.args.textnoserie:
            noserieEU = str(q.args.textnoserie)
            await q.page.save()

    if q.args.btnAddMod:
        if marcaEU != 'Seleccionar' and marcaEU != '':
            if modeloEU != 'Seleccionar' and modeloEU != '':
                if descripcionEU != 'Seleccionar' and descripcionEU != '':
                    if garantiaEU != 'Seleccionar' and garantiaEU != '':
                        if motivoEU != 'Seleccionar' and motivoEU != '':
                            if noserieEU != '':
                                data_rows_keycount += 1
                                data_rows.append([str(data_rows_keycount), str(noserieEU), 'N/A', str(marcaEU), str(modeloEU), str(descripcionEU), str(garantiaEU), str(motivoEU), 'Almacen', 'Usado'])
    ...
    if q.args.btnCreateQR:
        ...
        if selectionated != None:
            if len(selectionated) == 1:
                ...
                devUsedToAlm(r, key_count, fell, 'NA', 'NA', str(marcareg), str(modeloreg), str(descripcionreg), data_rows_send[0][6], data_rows_send[0][1], qr_code, 'No Misc', data_rows_send[0][7])
    ...
    #icon_path, = site.upload(['/home/data/cassia-logo1.png'])
