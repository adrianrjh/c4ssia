...
async def paso1(q: Q):
    ...
    res = getAllMatsDevs(r, 'Materiales de construccion')
    marcaBefore = ''
    comboboxMarca = []
    ...
    q.page['combotextboxes'] = ui.section_card(
        ...
        subtitle='',
        items=[
            ui.combobox(name='combomarca', label='Marca', value=str(marcaMC), choices=comboboxMarca,trigger=True, disabled=False),
            ui.combobox(name='combomodelo', label='Modelo', value=str(modeloMC), choices=comboboxModelo,trigger=True, disabled=False),
            ui.combobox(name='combodescripcion', label='Descripción', value=str(descripcionMC), choices=comboboxDescripcion,trigger=True, disabled=False),
            ui.combobox(name='combogarantia', label='Garantia', value=str(garantiaMC), choices=comboboxGaran,trigger=True, disabled=False),
            ui.combobox(name='combocantidad', label='Cantidad', value=str(cantidadMC), choices=comboboxCantidad,trigger=True, disabled=False),
    ...
    q.page['lista'] = ui.form_card(box=ui.box('der1_21', order=1), items=[
        ui.text_xl(content='Lista de Materiales de Construcción'),
        ui.table(
            name='issues',
            multiple = True,
            columns=columnsRegMC,
            rows=[ui.table_row(
                name=str(dato[0]),
                cells=dato,
...
async def sims_altaMC(q: Q):
    print(str("starting sims_altaMC..."))
    global ipGlobal, session, username
    global data_rows, data_rows_keycount
    global marcaMC, modeloMC, descripcionMC, garantiaMC, noserieMC, cantidadMC
    global paso1P, paso2P, paso3P, paso4P, bandPaso1, bandPaso2, bandPaso3, bandPaso4
    ...
    if 'combomarca' in q.args:
        if q.args.combomarca and str(q.args.combomarca) != 'Seleccionar':
            marcaMC = str(q.args.combomarca)
            res = getAllMatsMarca(r, marcaMC)
            modeloBefore = ''
            comboboxModelo = []
            for x in range(0, len(res)):
    ...
    if 'combomodelo' in q.args:
        if q.args.combomodelo and str(q.args.combomodelo) != 'Seleccionar':
            modeloMC = str(q.args.combomodelo)
            res = getAllMatsModelo(r, modeloMC)
            descripcionBefore = ''
            comboboxDescripcion = []
            for x in range(0, len(res)):
    ...
    if 'combodescripcion' in q.args:
        if q.args.combodescripcion and str(q.args.combodescripcion) != 'Seleccionar':
            descripcionMC = str(q.args.combodescripcion)
            paso1P = 1
            paso2P = 0
            paso3P = 0
            paso4P = 0
    ...
    if 'combogarantia' in q.args:
        if q.args.combogarantia and str(q.args.combogarantia) != 'Seleccionar':
            garantiaMC = str(q.args.combogarantia)
            paso1P = 1
            paso2P = 0
            paso3P = 0
            paso4P = 0
    ...
    if 'combocantidad' in q.args:
        if q.args.combocantidad and str(q.args.combocantidad) != 'Seleccionar':
            cantidadMC = str(q.args.combocantidad)
            paso1P = 1
            paso2P = 0
            paso3P = 0
            paso4P = 0
    ...
    if 'textnoserie' in q.args:
        if q.args.textnoserie:
            noserieMC = str(q.args.textnoserie)
            await q.page.save()

    if q.args.btnAddMod:
        if marcaMC != 'Seleccionar' and marcaMC != '':
            if modeloMC != 'Seleccionar' and modeloMC != '':
                if descripcionMC != 'Seleccionar' and descripcionMC != '':
                    if garantiaMC != 'Seleccionar' and garantiaMC != '':
                        if cantidadMC != 'Seleccionar' and cantidadMC != '':
                            if noserieMC != '':
                                data_rows_keycount += 1
                                data_rows.append([str(data_rows_keycount), str(noserieMC), 'N/A', str(marcaMC), str(modeloMC), str(descripcionMC), str(garantiaMC), str(cantidadMC), 'Almacen', 'Nuevo'])
    ...
    if q.args.btnCreateQR:
        ...
        data_rows_temp, data_rows_send, found, counter_article = [], [], 0, 0
        ...
        if selectionated != None:
            if len(selectionated) == 1:
                ...
                devNewToAlm(r, key_count, fell, 'NA', 'NA', str(marcareg), str(modeloreg), str(descripcionreg), data_rows_send[0][6], data_rows_send[0][1], qr_code, 'Misc', data_rows_send[0][9])
                data_to_report.append([fell, 'NA', 'NA', str(marcareg), str(modeloreg), str(descripcionreg), data_rows_send[0][6], data_rows_send[0][1], qr_code, 'Misc', data_rows_send[0][9]])
                data_rows = data_rows_temp
                if len(data_rows) > 0:
                    paso1P = 0
                    paso2P = 0
    ...
    if q.args.btnEndRecep:
        ...
        try:
            r.publish("yi_pdfs_recepcion_matconstruccion",json.dumps(json_datos))
            time.sleep(0.3)
        except Exception as e:
            print(e)
        paso1P = 1
